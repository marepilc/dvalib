 ...
 */
export const floor: (v: number) => number = Math.floor

/**
 * Alias of the `Math.ceil`
 */
export const ceil: (v: number) => number = Math.ceil
...
export function centile(numbers: number[], c: number): number {
  const dataCopy = numbers.concat()
  dataCopy.sort(function (a, b) {
    return a - b
  })
  const pos = ((dataCopy.length + 1) * c) / 100
  const ix = floor(pos)
  if (ix === 0) {
    return dataCopy[ix]
...
export class Vector {
   ...
   */
  public norm(): Vector {
    const e1 = this._x / Math.sqrt(this._x * this._x + this.y * this.y)
    const e2 = this._y / Math.sqrt(this._x * this._x + this._y * this._y)
   ...
   */
  public norm2(): void {
    const e1 = this._x / Math.sqrt(this._x * this._x + this._y * this._y)
    const e2 = this._y / Math.sqrt(this._x * this._x + this._y * this._y)
    this._x = e1
    this._y = e2
  }
