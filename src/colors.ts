...
function str2rgb(col: string): ColorRGB {
  ...
  const rgx: RegExp = /^#+([a-fA-F\d]{6}|[a-fA-F\d]{3})$/
  if (rgx.test(col)) {
    if (col.length == 4) {
...
export function blend(color1: string, color2: string, proportion: number): string {
  ...
  const c1 = color1.indexOf('#') === 0 ? color1 : '#' + color1
  const c2 = color2.indexOf('#') === 0 ? color2 : '#' + color2
  const rgx: RegExp = /^#+([a-fA-F\d]{6}|[a-fA-F\d]{3})$/
  if (rgx.test(c1) && rgx.test(c2)) {
    const col1 = c1.length === 7 ? c1.slice(1) : c1
    const col2 = c2.length === 7 ? c2.slice(1) : c2
    const r1 = int(col1.slice(0, 2), 16)
    const r2 = int(col2.slice(0, 2), 16)
    const r = round((1 - proportion) * r1 + proportion * r2)
    const g1 = int(col1.slice(2, 4), 16)
    const g2 = int(col2.slice(2, 4), 16)
    const g = round((1 - proportion) * g1 + proportion * g2)
    const b1 = int(col1.slice(4), 16)
    const b2 = int(col2.slice(4), 16)
    const b = round((1 - proportion) * b1 + proportion * b2)
...
export function randomColor(): string {
  const r: string = hexStr(randomInt(0, 255))
  const g: string = hexStr(randomInt(0, 255))
  const b: string = hexStr(randomInt(0, 255))
  return '#' + r + g + b
}
...
export function linearGradient(x0: number, y0: number, x1: number, y1: number): CanvasGradient {
  if (dva.ctx) {
    return dva.ctx.createLinearGradient(x0, y0, x1, y1)
  } else {
